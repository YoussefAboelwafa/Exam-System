...
export class AdminCalendarComponent implements OnInit {
  ...
  change_percentage(value: any) {
    ...
    this.service
      .change_percentage(
        this.user_exam[this.index_change_user]._id_user,
        this.user_exam[this.index_change_user].exam_id,
        value
      )
      .subscribe((x) => {});
    this.close_card();
  ...
  onmonthSelected(event: Event) {
    ...
    if (selectedmonth != 'Month') {
      ...
      this.calendar = this.calendar.filter(
        (obj) => obj.month_number === selectedmonth
      );
    }
  }
